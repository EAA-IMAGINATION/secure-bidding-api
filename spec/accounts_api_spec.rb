...
describe 'API /api/v1/accounts' do
  ...
  it 'HAPPY: gets account metadata with GET /api/v1/accounts/:id' do
    ...
    get "/api/v1/accounts/#{account.id}", '', { 'HTTP_AUTHORIZATION' => "Bearer #{token}" }
  ...
  it 'HAPPY: searches accounts by email on GET /api/v1/accounts/search' do
    ...
    get "/api/v1/accounts/search?email=#{CGI.escape('search-email@example.com')}", '', { 'HTTP_AUTHORIZATION' => "Bearer #{token}" }
  ...
  it 'SAD: returns 404 for missing account id' do
    admin = SecureBidding::Account.new(username: 'missing-admin', system_role: 'admin')
    admin.set_password('my-secret-pass')
    admin.set_email('missing-admin@example.com')
    admin.save
    token = SecureBidding::AuthToken.tokenize(
      { account_id: admin.id, username: admin.username, system_role: admin.system_role },
      SecureBidding::AuthToken::ONE_HOUR
    ...
    get '/api/v1/accounts/00000000-0000-0000-0000-000000000000', '', { 'HTTP_AUTHORIZATION' => "Bearer #{token}" }
  ...
  it 'SAD: rejects search without email or phone criteria' do
    admin = SecureBidding::Account.new(username: 'search-admin', system_role: 'admin')
    admin.set_password('my-secret-pass')
    admin.set_email('search-admin@example.com')
    admin.save
    token = SecureBidding::AuthToken.tokenize(
      { account_id: admin.id, username: admin.username, system_role: admin.system_role },
      SecureBidding::AuthToken::ONE_HOUR
    ...
    get '/api/v1/accounts/search', '', { 'HTTP_AUTHORIZATION' => "Bearer #{token}" }
