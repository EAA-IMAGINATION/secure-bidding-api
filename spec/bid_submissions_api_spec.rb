...
describe 'API /api/v1/bid_submissions' do
  ...
  it 'HAPPY: returns all bid submissions metadata from GET /api/v1/bid_submissions' do
    ...
    admin.save
    admin_header = auth_header_for(admin)

    get '/api/v1/bid_submissions', '', admin_header
  ...
  it 'SAD: returns 400 for invalid payload' do
    bidder = create_account(username: 'invalid-bidder', email: 'invalid-bidder@example.com')
    ...
    response_body = JSON.parse(last_response.body)
    _(response_body['error']).must_equal 'project_id and plaintext_bid are required'
  end
  ...
  it 'HAPPY: returns bid submission metadata for an existing id' do
    ...
    bid_submission.save
    admin = SecureBidding::Account.new(username: 'meta-admin', system_role: 'admin')
    admin.set_password('my-secret-pass')
    admin.set_email('meta-admin@example.com')
    admin.save

    get "/api/v1/bid_submissions/#{bid_submission.id}", '', auth_header_for(admin)
  ...
  it 'SAD: rejects SQL injection string in project_id for bid submission creation' do
    bidder = create_account(username: 'sql-bidder', email: 'sql-bidder@example.com')
    payload = {
