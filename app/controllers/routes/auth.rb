  ...
  module Routes
    ...
    module Auth
      ...
      def self.handle_register(req, app)
        ...
        validation = Validation::RegisterAccount.new.call(data.transform_keys(&:to_sym))
        if validation.failure?
          return req.halt(400, { error: validation_error_message(validation) }.to_json)
        end

        username = validation[:username].strip
        ...
        account = SecureBidding::Account.new(username: username)
      ...
      def self.handle_verify(req, app)
        ...
        validation = Validation::VerifyAccount.new.call(data.transform_keys(&:to_sym))
        if validation.failure?
          return req.halt(400, { error: validation_error_message(validation) }.to_json)
        end

        registration_token = validation[:registration_token].strip
        password = validation[:password].strip
