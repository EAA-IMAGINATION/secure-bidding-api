...
module SecureBidding
  ...
  class Project < Sequel::Model(:projects)
    ...
    set_allowed_columns :title, :budget_cents, :state, :submission_deadline_at
    ...
    def submission_deadline_passed?
      return true if submission_deadline_at.nil?

      Time.now >= submission_deadline_at
    end

    def bid_count
      bid_submissions_dataset.count
    end
  end
end
